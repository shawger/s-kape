...
#Gets a collection of cards (3 cards for now...)
#cards is an array
def getCardCollection(cards,cardType='none',heading='',greyBack=False):

    searchPrefix = ""
    if cardType == 'none':
        searchPrefix = ""
    else:
        searchPrefix = cardType + ":"


    if greyBack:
        returnString = "<div class='row cards-imbed grey-background'><h2>" + heading + "</h2>"
    else:
        returnString = "<div class='row cards-imbed'><h2>" + heading + "</h2>"

    #Only return 3 cards
    i = 0
    for card in cards:
        if(i < 3):
            returnString = returnString + getCard(searchPrefix + card,"")
        i = i + 1
